...
def get_side_triangle(user_input):
    for i in user_input:
        if i == ',':
            res = user_input.split(',', len(user_input))
            a = res[0]
            b = res[1]
            c = res[2]
    if a ==str('x'):
        ...
        Perimeter = res+b+c
        return print("The Opposite Side = {}\nArea = {}\nPerimeter of Triangle = {}".format(res, Area, Perimeter))
    elif b == str('x'):
        a = int(b)
        c = int(c)
        res = ((c**2)-(a**2)**.5)
        ...
        Perimeter = a+res+c 
        return print("The Adjacent Side = {}\nArea = {}\nPerimeter of Triangle = {}".format(res, Area, Perimeter))
    elif c == str('x'):
        a = int(a)
        b = int(b)
        res = ((a**2)+(b**2)**.5)
        ...
        Perimeter = a+b+res 
        return print("The Hypotnuse Side = {}\nArea = {}\nPerimeter of Triangle = {}".format(res, Area, Perimeter))
        #not returning the second line 

def get_side_trig(user_input):
    for i in user_input:
        if i == ',' or i ==', ':
            res = user_input.split(', ', len(user_input))
            opposite = res[0]
            ...
            print(res)
            break
    if type(opposite)==str and opposite == str('x'):
        res = opposite + "/" + Hypotnuse
        res2 = N(sin(Angle))
        finalfres = abs(res2*int(Hypotnuse))
        trig_compute_lside = ((int(Hypotnuse)**2)-(finalfres**2))**.5
        trig_compute_perm = trig_compute_lside+int(Hypotnuse)+finalfres
        s = trig_compute_perm/2
        trig_compute_area = math.sqrt(s*(s-trig_compute_lside)*(s-finalfres)*(s-int(Hypotnuse)))
        print("Length of Side {} = {}\nValue of cos{}\nFinal Answer {} = {}".format(str(opposite), res, res2, str(opposite), finalfres))
        print("The Adjacent Side = {}\nArea = {}\nPerimeter of Triangle = {}".format(finalfres, trig_compute_area, trig_compute_perm))

    if type(Adjacent)==str and Adjacent == str('x'):
        res = Adjacent+"/"+Hypotnuse
        res2 = N(cos(Angle))
        finalfres = abs(res2*int(Hypotnuse))
        trig_compute_lside = ((int(Hypotnuse)**2)-(finalfres**2))**.5
        trig_compute_perm = trig_compute_lside+int(Hypotnuse)+finalfres
        ...
        trig_compute_area = math.sqrt(s*(s-trig_compute_lside)*(s-finalfres)*(s-int(Hypotnuse)))
        print("Length of Side {} = {}\nValue of cos{}\nFinal Answer {} = {}".format(str(opposite), res, res2, str(opposite), finalfres))
        print("The Opposite Side = {}\nArea = {}\nPerimeter of Triangle = {}".format(finalfres, trig_compute_area, trig_compute_perm))

    if type(Hypotnuse)==str and Hypotnuse == str('x'):
        res = Hypotnuse+"/"+Adjacent
        res2 = N(cos(Angle))
        finalfres = abs(res2*int(Adjacent))
        trig_compute_lside = ((int(Hypotnuse)**2)-(finalfres**2))**.5
        trig_compute_perm = trig_compute_lside+int(Hypotnuse)+finalfres
        s = trig_compute_perm/2
        trig_compute_area = math.sqrt(s*(s-trig_compute_lside)*(s-finalfres)*(s-int(Hypotnuse)))
        print("Length of Side {} = {}\nValue of cos{}\nFinal Answer {} = {}".format(str(opposite), res, res2, str(opposite), finalfres))
        print("The Opposite Side = {}\nArea = {}\nPerimeter of Triangle = {}".format(finalfres, trig_compute_area, trig_compute_perm))
    if type(opposite)==int and type(Adjacent)==int and type(Hypotnuse)==int:
        raise TypeError ("User Has Filled All Parameters with ")
    if type(Angle)==str:
        raise TypeError (f"User Has Enter A String '{Angle}' of Angle")
    else:
        pass